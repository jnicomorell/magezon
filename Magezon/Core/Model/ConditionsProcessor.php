...
class ConditionsProcessor
{
    ...
    /**
     * Retrieve product collection filtered by conditions
     *
    ...
    public function getProductByConditions($model, $store)
    {
        ...
        // Agrega atributos necesarios para las condiciones (si corresponde)
        $conditions = $model->getConditions();
        if ($conditions) {
            $conditions->collectValidatedAttributes($collection);

            // En lugar de usar el Sql\Builder (no-API), iteramos manualmente y aplicamos filtros simples.
            // Aquí es donde debes adaptar tu propia lógica de filtrado.
            $validatedConditions = $conditions->getConditions();
            if (is_array($validatedConditions)) {
                foreach ($validatedConditions as $condition) {
                    // Ejemplo: supongamos que las condiciones usan atributos estándar
                    // ¡Debes adaptar esto a tu lógica real de condiciones!
                    if ($condition->getAttribute() && $condition->getOperator() && $condition->getValue()) {
        ...
        // Lógica de filtrado por tipo de producto
        ...
        } elseif ($model->getStockStatus() === "2") {
            $this->addInStockFilterToCollection($collection);
        }

        // Join para obtener la cantidad en stock
    ...
    public function addInStockFilterToCollection($collection)
    {
        ...
        $collection->addAttributeToFilter('inventory_in_stock', ['eq' => 1]);
    }
    ...
    public function addOutStockFilterToCollection($collection)
    {
        ...
        $collection->addAttributeToFilter('inventory_in_stock', ['eq' => 0]);
    }
